...
```bash
# Generate VOTD audio + video with burned-in captions (on by default, 2x font)
python scripts/gen_votd_edge.py -i input.txt --mp4

# Generate VOTD audio + video without captions
python scripts/gen_votd_edge.py -i input.txt --mp4 --no-caption

# Custom caption scale (1x standard, 3x large)
python scripts/gen_votd_edge.py -i input.txt --mp4 --caption-scale 1x
python scripts/gen_votd_edge.py -i input.txt --mp4 --caption-scale 3x

# Or convert any existing audio to MP4 (2x captions by default)
